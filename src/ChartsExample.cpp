  ...
  WContainerWidget()
{
  ...
  //std::string dataPath = "resources/category.csv";
  std::string dataPath = "data/out_file_s470017560_1565989934914.csv";

  // Read the CSV file
  std::shared_ptr<WAbstractItemModel> model = readCsvFile(WApplication::appRoot() + dataPath, this);
  ...
  WContainerWidget()
{
  ...
  std::shared_ptr<WAbstractItemModel> model
      = readCsvFile(WApplication::appRoot() + "resources/" + "timeseries.csv", this);
