...
double SyncDeviceTimes(int handle) {
	...
	LJMError = LJM_eReadAddress(handle, 61500, 1, &labjackTime);
	ErrorCheck(LJMError, "LJM_eReadAddress");
	//printf("LABJACK EPOCH_TIME: %d\n", labjackTime);
	time_t originalLabjackTime = labjackTime;
	...
	if (updateChangeSeconds == 0) {
		...
		LJMError = LJM_eWriteAddress(handle, 61504, LJM_UINT32, computerTime);
		ErrorCheck(LJMError, "LJM_eWriteAddress");

		// Re-read the time to confirm the update
		LJMError = LJM_eReadAddress(handle, 61500, 1, &labjackTime);
		ErrorCheck(LJMError, "LJM_eReadAddress");
		//printf("Updated Labjack EPOCH_TIME: %d\n", labjackTime);
		time_t updatedLabjackTime = labjackTime;
