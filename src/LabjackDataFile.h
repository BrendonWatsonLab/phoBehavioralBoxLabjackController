...
struct LabjackDataFile {
	...
	// This could parse its variables exclusively from fullPath, but that would be less efficient for the "BehavioralBoxLajbackController::findDataFiles((std::string searchDirectory, int labjackSerialNumber, ...)" approach which can skip a file if it doesn't match the labjack
